...
async function api(action,extra={}){
 const r=await fetch(base+'/api/game',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,...extra})});
 const d=await r.json();assert.equal(r.status,200,JSON.stringify(d));return d;
}
const host=await api('create',{name:'HTTP 방장'}), credentials=[{code:host.code,token:host.token}];
for(let i=1;i<15;i++){const p=await api('join',{code:host.code,name:`HTTP ${i}`});credentials.push({code:host.code,token:p.token});}
await api('start',credentials[0]);await sleep(3350);
const start=await Promise.all(credentials.map(c=>api('sync',c)));
assert.equal(start.filter(s=>s.hint!==null).length,3);assert.ok(start.every(s=>s.players.length===15));
const begin=performance.now();
const presses=await Promise.all(credentials.map(c=>api('press',{...c,round:start[0].round,gate:start[0].gate})));
await sleep(250);
const end=await Promise.all(credentials.map(c=>api('sync',c)));
assert.ok(end.every(s=>s.phase==='result'&&s.result.reason==='CRASH'));
